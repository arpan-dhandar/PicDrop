...
const Feed = () => {
  ...
  useEffect(() => {
    // FIXED: Now fetching from Render
      ...
      <main className="px-8 md:px-12 pb-20">
        {posts.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-6 gap-y-10">
            {posts.map((post) => (
              <CardUi key={post._id} image={post.image} caption={post.caption} />
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center h-[60vh] text-center">
            ...
            <button className="px-8 py-3 border border-zinc-800 rounded-2xl hover:border-zinc-500 transition-colors" onClick={() => navigate("/")}>
              Create First Drop
            </button>
          </div>
        )}
